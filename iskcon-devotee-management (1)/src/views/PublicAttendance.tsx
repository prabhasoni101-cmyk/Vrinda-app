...
import { collection, query, where, onSnapshot, doc, setDoc, getDoc, updateDoc, increment, getDocs, addDoc } from 'firebase/firestore';
...
export default function PublicAttendance() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      const q = query(collection(db, 'devotees'), where('contact', '==', contact));
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Find or Create/Update Devotee
      const qDevotee = query(collection(db, 'devotees'), where('contact', '==', normalizedContact));
      const devoteeSnap = await getDocs(qDevotee);
      
      let devoteeId = '';
      const facilitatorName = templeUsers.find(u => u.uid === form.facilitatorId)?.displayName || form.facilitator || 'Self';
      ...
      if (!devoteeSnap.empty) {
        // Update existing devotee
        devoteeId = devoteeSnap.docs[0].id;
        ...
        // Create new devotee
        const newDevRef = await addDoc(collection(db, 'devotees'), {
          ...devoteeData,
          attendanceCount: 1,
          createdAt: new Date().toISOString()
        });
        devoteeId = newDevRef.id;
      }

      // 2. Mark Attendance for this Event
      const attRef = doc(db, `events/${id}/attendance`, devoteeId);
      await setDoc(attRef, {
        ...devoteeData,
        devoteeId,
        present: true,
      ...
      // Persist to local storage for future auto-fills
